...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        try {
            $imagem = $produto['imagem']; // Manter imagem atual
            ...
            if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] === UPLOAD_ERR_OK) {
                ...
                // Mover arquivo
                if (move_uploaded_file($arquivo['tmp_name'], $diretorio . '/' . $nova_imagem)) {
                    // Excluir imagem antiga
                    if ($imagem && file_exists($diretorio . '/' . $imagem)) {
            ...
            $stmt = $pdo->prepare("
                UPDATE produtos 
                SET nome = ?, descricao = ?, preco = ?, categoria_id = ?, imagem = ?, ativo = ?
                WHERE id = ?
            ");
            ...
            redirecionarComMensagem('listar.php', 'Produto atualizado com sucesso!', 'success');
            
        } catch (Exception $e) {
            // Se houver erro e foi feito upload de nova imagem, excluir
            if (isset($nova_imagem) && file_exists('../../uploads/produtos/' . $nova_imagem)) {
                unlink('../../uploads/produtos/' . $nova_imagem);
            }
            ...
            <div class="admin-form">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="preco" name="preco" class="campo-moeda" value="<?php echo number_format($produto['preco'], 2, ',', '.'); ?>" required>
                    </div>
